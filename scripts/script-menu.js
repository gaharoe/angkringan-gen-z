let menuItems = []
    ...
    function renderMenu(items = menuItems) {
      ...
      if (items.length === 0) {
        menuGrid.innerHTML =
          '<div class="col-span-full text-center py-10 text-gray-500">No items found</div>';
        return;
      }
      items.forEach((item) => {
    ...
    $id("maintenance").addEventListener("click", () => {
      $id("maintenance").classList.add("hidden")
    });
    ...
    checkoutBtn.addEventListener("click", () => {
      $id("maintenance").classList.remove("hidden")
      ...
      // const message = `${cart.map((item) => `- ${item.quantity}x ${item.name}`)} Total: Rp. ${total}`;
    ...
    document.addEventListener("DOMContentLoaded", () => {
      fetch("http://192.168.1.9/api/menus", {method: "GET", headers: {"Content-Type":"application/json"}})
      .then(req => req.json())
      .then(data => {
        menuItems = data; 
